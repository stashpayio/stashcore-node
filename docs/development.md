...
To develop bitcore-node-stash:

```bash
cd ~
git clone git@github.com:<yourusername>/bitcore-node-stash.git
...
We now will setup symlinks in `bitcore-node-stash` *(repeat this for any other modules you're planning on developing)*:
```bash
cd node_modules
rm -rf bitcore-lib
ln -s ~/bitcore-lib
rm -rf bitcoind-rpc-stash
ln -s ~/bitcoind-rpc-stash
```
...
cd devnode
mkdir node_modules
touch bitcore-node-stash.json
touch package.json
```

Edit `bitcore-node-stash.json` with something similar to:
```json
{
  "network": "livenet",
  "port": 3001,
...
```bash
../bitcore-node-stash/bin/bitcore-node-stash start
```