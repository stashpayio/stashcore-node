...
var StashcoreNode = require('../node');
var index = require('../');
...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 'A reindex for stashd is necessary for this upgrade with the "reindex=1" stash.conf option. \n' +
                 'There are changes necessary in both stash.conf and stashcore-node.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/stashpayio/stashcore-node/blob/stashd/docs/upgrade.md \n');
        ...
        servicesConfig: {
          stashd: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/stashd')
            }
          }
        }
      };
 ...
 * @param {Object} options.config - The parsed stashcore-node.json configuration file
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './stashcore-node.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in stashcore-node services directory
    ...
    if (servicePackage.stashcoreNode) {
      serviceModule = service.name + '/' + servicePackage.stashcoreNode;
    }
 ...
 * @param {Node} node - The Stashcore Node instance
 */
