...
/**
 * Will remove a service from stashcore-node.json
...
/**
 * Will remove the Node.js service and from the stashcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The stashcore-node configuration directory
...
function remove(options, done) {
  ...
  var stashcoreConfigPath = path.resolve(configPath, 'stashcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(stashcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a stashcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to stashcore-node.json
        removeConfig(stashcoreConfigPath, service, next);
      });
    }, done
  );
}
