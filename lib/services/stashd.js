...
Stash.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  var txids;
...
Stash.prototype.getDetailedTransaction = function(txid, callback) {
  ...
  function addTxlockToTx(tx,result) {
    tx.txlock = false;
    if (tx.feeSatoshis >= Stash.DEFAULT_INSTANTSEND_FEE) { // if transaction fee is >= 0.0001 check for txlock
      var rawTx = new stashcore.Transaction(result.hex);
      var hash = stashcore.crypto.Hash.sha256sha256(rawTx.toBuffer());
      var id = hash.toString('binary');
      if (self.node.services.stashd.zmqKnownTransactionLocks.get(id)) {
