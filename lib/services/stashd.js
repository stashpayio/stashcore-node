...
/**
 * Provides a friendly event driven API to stashd in Node.js. Manages starting and
 * stopping stashd as a child process for application support, as well as connecting
 * to multiple stashd processes for server infrastructure. Results are cached in an
...
util.inherits(Stash, Service);

Stash.dependencies = [];

Stash.DEFAULT_MAX_TXIDS = 1000;
Stash.DEFAULT_MAX_HISTORY = 50;
Stash.DEFAULT_MAX_UTXO = 1000;
Stash.DEFAULT_SHUTDOWN_TIMEOUT = 15000;
Stash.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS = 0.9999;
Stash.DEFAULT_MAX_ADDRESSES_QUERY = 10000;
Stash.DEFAULT_SPAWN_RESTART_TIME = 5000;
Stash.DEFAULT_SPAWN_STOP_TIME = 10000;
Stash.DEFAULT_TRY_ALL_INTERVAL = 1000;
Stash.DEFAULT_REINDEX_INTERVAL = 10000;
Stash.DEFAULT_START_RETRY_INTERVAL = 5000;
Stash.DEFAULT_TIP_UPDATE_INTERVAL = 15000;
Stash.DEFAULT_TRANSACTION_CONCURRENCY = 5;
Stash.DEFAULT_INSTANTSEND_FEE = 10000;
Stash.DEFAULT_CONFIG_SETTINGS = {
  server: 1,
  whitelist: '127.0.0.1',
  txindex: 1,
  addressindex: 1,
...
Stash.prototype._initDefaults = function(options) {
  /* jshint maxcomplexity: 15 */

  // limits
  this.maxTxids = options.maxTxids || Stash.DEFAULT_MAX_TXIDS;
  this.maxTransactionHistory = options.maxTransactionHistory || Stash.DEFAULT_MAX_HISTORY;
  this.maxUTXOHistory = options.maxUTXOHistory || Stash.DEFAULT_MAX_UTXO;
  this.maxAddressesQuery = options.maxAddressesQuery || Stash.DEFAULT_MAX_ADDRESSES_QUERY;
  this.shutdownTimeout = options.shutdownTimeout || Stash.DEFAULT_SHUTDOWN_TIMEOUT;

  // spawn restart setting
  this.spawnRestartTime = options.spawnRestartTime || Stash.DEFAULT_SPAWN_RESTART_TIME;
  this.spawnStopTime = options.spawnStopTime || Stash.DEFAULT_SPAWN_STOP_TIME;

  // try all interval
  this.tryAllInterval = options.tryAllInterval || Stash.DEFAULT_TRY_ALL_INTERVAL;
  this.startRetryInterval = options.startRetryInterval || Stash.DEFAULT_START_RETRY_INTERVAL;

  // rpc limits
  this.transactionConcurrency = options.transactionConcurrency || Stash.DEFAULT_TRANSACTION_CONCURRENCY;

  // sync progress level when zmq subscribes to events
  this.zmqSubscribeProgress = options.zmqSubscribeProgress || Stash.DEFAULT_ZMQ_SUBSCRIBE_PROGRESS;
};

Stash.prototype._initCaches = function() {
...
Stash.prototype._initClients = function() {
  var self = this;
  this.nodes = [];
  this.nodesIndex = 0;
 ...
 */
Stash.prototype.getAPIMethods = function() {
  var methods = [
...
Stash.prototype.subscribe = function(name, emitter) {
  this.subscriptions[name].push(emitter);
  log.info(emitter.remoteAddress, 'subscribe:', 'stashd/' + name, 'total:', this.subscriptions[name].length);
};

Stash.prototype.unsubscribe = function(name, emitter) {
  var index = this.subscriptions[name].indexOf(emitter);
  if (index > -1) {
    this.subscriptions[name].splice(index, 1);
  }
  log.info(emitter.remoteAddress, 'unsubscribe:', 'stashd/' + name, 'total:', this.subscriptions[name].length);
};

Stash.prototype.subscribeAddress = function(emitter, addresses) {
  var self = this;
  ...
  if(addresses){
    for(var i = 0; i < addresses.length; i++) {
      if (stashcore.Address.isValid(addresses[i], this.node.network)) {
        addAddress(addresses[i]);
      }
    }
  }

  log.info(emitter.remoteAddress, 'subscribe:', 'stashd/addresstxid', 'total:', _.size(this.subscriptions.address));
};

Stash.prototype.unsubscribeAddress = function(emitter, addresses) {
  var self = this;
  if(!addresses) {
  ...
  log.info(emitter.remoteAddress, 'unsubscribe:', 'stashd/addresstxid', 'total:', _.size(this.subscriptions.address));
};
 ...
 */
Stash.prototype.unsubscribeAddressAll = function(emitter) {
...
Stash.prototype._getDefaultConfig = function() {
  var config = '';
  var defaults = Stash.DEFAULT_CONFIG_SETTINGS;
...
Stash.prototype._parseStashConf = function(configPath) {
  var options = {};
...
Stash.prototype._expandRelativeDatadir = function() {
...
Stash.prototype._loadSpawnConfiguration = function(node) {
  /* jshint maxstatements: 25 */

  $.checkArgument(this.options.spawn, 'Please specify "spawn" in stashd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in stashd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in stashd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './stash.conf');

  log.info('Using Stash config file:', configPath);
  ...
  _.extend(this.spawn.config, this._parseStashConf(configPath));

  var networkConfigPath = this._getNetworkConfigPath();
  if (networkConfigPath && fs.existsSync(networkConfigPath)) {
    _.extend(this.spawn.config, this._parseStashConf(networkConfigPath));
  }
...
Stash.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    spawnConfig.txindex && spawnConfig.txindex === 1,
    '"txindex" option is required in order to use transaction query features of stashcore-node. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of stashcore-node. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of stashcore-node. ' +
  ...
  $.checkState(
    ...
    '"zmqpubrawtx" and "zmqpubhashblock" are expected to the same host and port in stash.conf'
  );

  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that stashd is undergoing a reindex. ' +
             ...
             'of stashcore-node services will start.');
    node._reindex = true;
  }
};

Stash.prototype._resetCaches = function() {
 ...
 * @private
 */
Stash.prototype._tryAllClients = function(func, callback) {
  var self = this;
...
Stash.prototype._wrapRPCError = function(errObj) {
  var err = new errors.RPCError(errObj.message);
  err.code = errObj.code;
  return err;
};

Stash.prototype._initChain = function(callback) {
  var self = this;
...
Stash.prototype._getDefaultConf = function() {
  var networkOptions = {
    rpcport: 9998
  };
  if (this.node.network === stashcore.Networks.testnet) {
    networkOptions.rpcport = 19998;
  }
  return networkOptions;
};

Stash.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === stashcore.Networks.testnet) {
    networkPath = 'testnet3/stash.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/stash.conf';
    }
  }
  return networkPath;
};

Stash.prototype._getNetworkOption = function() {
  var networkOption;
  if (this.node.network === stashcore.Networks.testnet) {
...
Stash.prototype._zmqBlockHandler = function(node, message) {
  var self = this;
  ...
  if (!self.zmqKnownBlocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.hashblock.length; i++) {
      this.subscriptions.hashblock[i].emit('stashd/hashblock', message.toString('hex'));
    }
  }

};

Stash.prototype._rapidProtectedUpdateTip = function(node, message) {
  var self = this;
...
Stash.prototype._updateTip = function(node, message) {
  var self = this;
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('Stash Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
...
Stash.prototype._getAddressesFromTransaction = function(transaction) {
  var addresses = [];
...
Stash.prototype._notifyAddressTxidSubscribers = function(txid, transaction) {
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('stashd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Stash.prototype._zmqTransactionHandler = function(node, message) {
  var self = this;
  var hash = stashcore.crypto.Hash.sha256sha256(message);
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('stashd/rawtransaction', message.toString('hex'));
    }

    var tx = stashcore.Transaction();
    tx.fromString(message);
    var txid = stashcore.util.buffer.reverse(hash).toString('hex');
    self._notifyAddressTxidSubscribers(txid, tx);

  }
};

Stash.prototype._zmqTransactionLockHandler = function(node, message) {
  var self = this;
  var hash = stashcore.crypto.Hash.sha256sha256(message);
  var id = hash.toString('binary');
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('stashd/transactionlock', message.toString('hex'));
    }
...
Stash.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  var self = this;
  var interval;
  ...
  checkAndSubscribe(function(err, synced) {
    ...
    if (!synced) {
      ...
      }, node._tipUpdateInterval || Stash.DEFAULT_TIP_UPDATE_INTERVAL);
    }
  });

};

Stash.prototype._subscribeZmqEvents = function(node) {
  var self = this;
...
Stash.prototype._initZmqSubSocket = function(node, zmqUrl) {
  var self = this;
...
Stash.prototype._checkReindex = function(node, callback) {
  var self = this;
  var interval;
  function finish(err) {
    clearInterval(interval);
  ...
  interval = setInterval(function() {
    node.client.getBlockchainInfo(function(err, response) {
      ...
      log.info('Stash Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
  ...
  }, node._reindexWait || Stash.DEFAULT_REINDEX_INTERVAL);

};

Stash.prototype._loadTipFromNode = function(node, callback) {
  var self = this;
  node.client.getBestBlockHash(function(err, response) {
    if (err && err.code === -28) {
...
Stash.prototype._stopSpawnedStash = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/stashd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned stash process with pid: ' + pid);
        self._process.kill(pid, 'SIGINT');
      } catch(err) {
        if (err && err.code === 'ESRCH') {
          log.warn('Unclean stash process shutdown, process not found with pid: ' + pid);
          return callback(null);
        } else if(err) {
          return callback(err);
        }
...
Stash.prototype._spawnChildProcess = function(callback) {
  var self = this;
  ...
  self._stopSpawnedStash(function(err) {
    if (err) {
      return callback(err);
    }

    log.info('Starting stash process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('Stash process unexpectedly exited with code:', code);
        log.warn('Restarting stash child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('Stash process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn stashd.'));
      }
...
Stash.prototype._connectProcess = function(config, callback) {
  var self = this;
  var node = {};
  var exitShutdown = false;
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to stashd.'));
    }
 ...
 */
Stash.prototype.start = function(callback) {
  var self = this;
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('Stash configuration options "spawn" or "connect" are expected'));
    }
 ...
 * @param {Function} callback
 */
Stash.prototype.isSynced = function(callback) {
 ...
 * @param {Function} callback
 */
Stash.prototype.syncPercentage = function(callback) {
  var self = this;
...
Stash.prototype._normalizeAddressArg = function(addressArg) {
 ...
 * @param {String|Address|Array} addressArg - An address string, stashcore address, or array of addresses
 * @param {Object} options
 * @param {Function} callback
 */
Stash.prototype.getAddressBalance = function(addressArg, options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.getAddressUnspentOutputsPaginated = function(addressArg, options, callback) {
  var self = this;
  ...
  function transformUnspentOutput(delta) {
    var script = stashcore.Script.fromAddress(delta.address);
    return {
 ...
 * @param {Function} callback
 */
Stash.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var addresses = self._normalizeAddressArg(addressArg);
  var cacheKey = addresses.join('');
...
Stash.prototype._getBalanceFromMempool = function(deltas) {
  var satoshis = 0;
...
Stash.prototype._getTxidsFromMempool = function(deltas) {
  var mempoolTxids = [];
...
Stash.prototype._getHeightRangeQuery = function(options, clone) {
  if (options.start >= 0 && options.end >= 0) {
    if (options.end > options.start) {
      throw new TypeError('"end" is expected to be less than or equal to "start"');
    }
    if (clone) {
      // reverse start and end as the order in stashcore is most recent to less recent
 ...
 * @param {Function} callback
 */
Stash.prototype.getAddressTxids = function(addressArg, options, callback) {
...
Stash.prototype._getConfirmationsDetail = function(transaction) {
...
Stash.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  if (!input.address) {
    return;
  }
...
Stash.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  if (!output.address) {
    return;
  }
...
Stash.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  var result = {
    addresses: {},
    satoshis: 0
  };
 ...
 * @param {Object} txid - A stash transaction id
 * @param {Function} callback
 */
Stash.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
...
Stash.prototype._getAddressStrings = function(addresses) {
  var addressStrings = [];
  for (var i = 0; i < addresses.length; i++) {
    var address = addresses[i];
    if (address instanceof stashcore.Address) {
...
Stash.prototype._paginate = function(fullArray, fromArg, toArg, fromHeight, toHeight) {
  var slicedArray;
  var filteredArray;
 ...
 * @param {Function} callback
 */
Stash.prototype.getAddressHistory = function(addressArg, options, callback) {
  var self = this;
  var addresses = self._normalizeAddressArg(addressArg);
 ...
 * @param {Function} callback
 */
Stash.prototype.getAddressSummary = function(addressArg, options, callback) {
  var self = this;
  var summary = {};
  var queryMempool = _.isUndefined(options.queryMempool) ? true : options.queryMempool;
  var summaryTxids = [];
 ...
 * @private
 */
Stash.prototype._maybeGetBlockHash = function(blockArg, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.getRawBlock = function(blockArg, callback) {
 ...
 * @param {Function} callback
 */
Stash.prototype.getBlockOverview = function(blockArg, callback) {
  var self = this;
...
/**
 * Will retrieve a block as a Stashcore object
 * @param {String|Number} block - A block hash or block height number
 * @param {Function} callback
 */
Stash.prototype.getBlock = function(blockArg, callback) {
  // TODO apply performance patch to the RPC method for raw data
  var self = this;
    ...
    if (cachedBlock) {
      ...
      self._tryAllClients(function(client, done) {
        client.getBlock(blockhash, false, function(err, response) {
          ...
          var blockObj = stashcore.Block.fromString(response.result);
 ...
 * @param {Function} callback
 */
Stash.prototype.getBlockHashesByTimestamp = function(high, low, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
 ...
 */
Stash.prototype.getBlockHeaders = function(blockArg, callback, nbOfBlockToFetch) {
	var self = this;
	var _toFetch = 25;
 ...
 * @param {Function} callback
 */
Stash.prototype.estimateFee = function(blocks, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.sendTransaction = function(tx, options, callback) {
  var self = this;
...
Stash.prototype._getRawTransactionData = function(txid, callback) {
  var self = this;
  self._tryAllClients(function(client, done) {
 ...
 * @param {Function} callback
 */
Stash.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
...
/**
 * Will get a transaction as a Stashcore Transaction. Results include the mempool.
 * @param {String} txid - The transaction hash
 * @param {Boolean} queryMempool - Include the mempool
 * @param {Function} callback
 */
Stash.prototype.getTransaction = function(txid, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  ...
  function addJoinSplitsToTx(tx, result) {
    ...
    if (result && result.vjoinsplit) {
      for (var jsIndex = 0; jsIndex < result.vjoinsplit.length; jsIndex++) {
        var jsdesc = result.vjoinsplit[jsIndex];
        var vpub_oldZat = jsdesc.vpub_new * (10 ^ 8);
        var vpub_newZat = jsdesc.vpub_old * (10 ^ 8);
        netJoinSplitZatoshis += vpub_newZat - vpub_oldZat;
        tx.joinSplits.push({
          oldZatoshis: vpub_oldZat,
          newZatoshis: vpub_newZat,
        });
      }
      
      // Quick dirty fix for fee amount 
      // TODO read from actual joinsplits
      return tx.joinSplits.length * 0.0001;
    }
  ...
  if (tx) {
    ...
    self._tryAllClients(function(client, done) {
      client.getRawTransaction(txid, 1, function(err, response) {
        ...
        else if (result.version == 1 && result.vout && !result.joinSplits) {
          tx.legacy = true;
        }
 ...
 * @param callback
 */
Stash.prototype.govObjectList = function(options, callback) {
  var self = this;
...
Stash.prototype.getCurrentVotes = function (govhash, callback) {
  var self = this;
...
Stash.prototype.getVotes = function (govhash, callback) {
  var self = this;
...
Stash.prototype.getSuperBlockBudget = function (blockindex, callback) {
  var self = this;
...
/*jshint maxparams: 6 */
Stash.prototype.govObjectSubmit = function (parentHash, revision, time, dataHex, feeTxId, callback) {
  var self = this;
...
Stash.prototype.govObjectDeserialize = function (hexdata, callback) {
  var self = this;
...
Stash.prototype.govObjectCheck = function(hexdata, callback){
  var self = this;
...
Stash.prototype.govObjectInfo = function (callback) {
  var self = this;
...
Stash.prototype.govCount = function (callback) {
  var self = this;
...
Stash.prototype.getSpork = function(callback){
	var self = this;
	var SporksList = {};
...
Stash.prototype.getMNList = function(callback){
  var self = this;
  var rawResults= {};
  var MNList = [];
 ...
 * @param callback
 */
Stash.prototype.govObjectHash = function(hash, callback) {
  var self = this;
 ...
 * @param callback
 */
Stash.prototype.govObjectCheck = function govObjectCheck(hexdata, callback) {
  var self = this;
  this.client.gobject('check', hexdata, function (err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
 ...
 * @param {Function} callback
 */
Stash.prototype.getBestBlockHash = function(callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.getSpentInfo = function(options, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.getInfo = function(callback) {
  var self = this;
...
Stash.prototype.generateBlock = function(num, callback) {
  var self = this;
 ...
 * @param {Function} callback
 */
Stash.prototype.stop = function(callback) {
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('stashd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('stashd process did not exit'));
      }
